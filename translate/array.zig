...
pub export fn int_sort(arg_array: [*c]c_int, arg_count: usize) void {
    var array = arg_array;
    _ = &array;
    var count = arg_count;
    _ = &count;
    {
        var i: c_int = 0;
        _ = &i;
        while (@as(usize, @bitCast(@as(c_long, i))) < (count -% @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))))) : (i += 1) {
            {
                var j: c_int = 0;
                _ = &j;
                while (@as(usize, @bitCast(@as(c_long, j))) < ((count -% @as(usize, @bitCast(@as(c_long, i)))) -% @as(usize, @bitCast(@as(c_long, @as(c_int, 1)))))) : (j += 1) {
                    if ((blk: {
                        const tmp = j;
                        if (tmp >= 0) break :blk array + @as(usize, @intCast(tmp)) else break :blk array - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                    }).* > (blk: {
...
// /home/lloydf/zig/zig-linux-x86_64-0.14.0/lib/include/__stddef_offsetof.h:16:9
